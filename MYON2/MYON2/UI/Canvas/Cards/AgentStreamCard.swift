    ...
    @ViewBuilder private func row(for step: AgentStreamStep) -> some View {
        HStack(alignment: .top, spacing: Space.sm) {
            ...
            VStack(alignment: .leading, spacing: Space.xs) {
                switch step.kind {
                case .thinking:
                    MyonText("Thinking…", style: .subheadline, color: ColorsToken.Text.secondary)
                    ProgressView().progressViewStyle(.circular)
                case .info:
                    MyonText(step.text ?? "", style: .body)
                case .lookup:
                    MyonText(step.text ?? "", style: .subheadline, color: ColorsToken.Text.secondary)
                case .result:
                    MyonText(step.text ?? "", style: .body)
                }
            }
