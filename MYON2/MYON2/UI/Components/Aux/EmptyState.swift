...
public struct EmptyState: View {
    ...
    public var body: some View {
        VStack(spacing: Space.md) {
            ...
            MyonText(title, style: .title2)
            if let message { MyonText(message, style: .callout, color: ColorsToken.Text.secondary, align: .center) }
            if let actionTitle, let action {
                MyonButton(actionTitle, style: .primary, action: action)
                    .frame(maxWidth: 280)
            }
        }
