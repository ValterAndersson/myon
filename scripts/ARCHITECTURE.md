...
| `backfill_analysis_jobs.js` | Enqueue POST_WORKOUT and WEEKLY_REVIEW analysis jobs for historical data. Idempotent (deterministic job IDs). Run after `backfill_set_facts.js`. | `node scripts/backfill_analysis_jobs.js --user <userId> --months 3` |
| `find_analysis_users.js` | Find users with existing analysis data (analysis_insights, weekly_reviews). | `node scripts/find_analysis_users.js` |
| `dump_analysis_data.js` | Dump latest analysis insights and reviews for a user. | `node scripts/dump_analysis_data.js` |
