...
- Parallel fetch: `getActiveWorkout` + `getAnalysisSummary(daily_brief)` (~200ms)
- Sequential: `getExerciseSummary` for the current exercise (depends on workout data)
...
- Firebase endpoints called: `proposeCards`, `bootstrapCanvas`, `getPlanningContext`, `searchExercises`, `logSet`, `swapExercise`, `completeActiveWorkout`, `getActiveWorkout`, etc.
