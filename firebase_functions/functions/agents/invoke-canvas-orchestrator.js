...
exports.invokeCanvasOrchestrator = async (req, res) => {
  try {
    ...
    // Pre-publish a light info card so UI shows activity
    try {
    ...
    if (createResp.status >= 400) {
      functions.logger.error('create_session failed', { status: createResp.status, data: createResp.data });
    ...
    const contextHint = `(context: canvas_id=${canvasId} user_id=${userId}; if route=workout then call tool_workout_stage1_publish)`;
    const streamPayload = {
    ...
    functions.logger.info('streamQuery status', { status: streamResp.status });
