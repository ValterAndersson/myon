...
## B2) Intelligent autonomy without losing structure
- **Goal**: Let the pipeline reason about what is missing or suboptimal, but require the *actions* to follow a shared schema, constrained language, and positioning guidelines.
- **Approach**:
  - Keep rich LLM reasoning in the “analysis” sections of each tool but require the “actions” array to be strict JSON that references schema-validated fields (e.g., `field_path`, `before`, `after`, `evidence_tag`, `confidence`).
  - Add a **policy middleware** that accepts free-form reasoning but only executes actions passing schema checks, invariant checks, and positioning rules (canonical naming, tone, tense, equipment vocabulary).
  - Require every mutation proposal to include a deterministic **error/fix code** plus suggested fix instructions so the system can feed back precise corrections when validation fails.
  - Maintain an **LLM-visible change plan** (the action list) that is echoed back to the LLM on validation failure with concrete error messages, enabling the model to iteratively repair its own output while staying within schema.
...
- **Validation feedback**: On schema/policy rejection, return machine-actionable errors (code + path + message) to the LLM along with the last proposed action list so it can self-correct without changing already accepted fields.
...
## E2) Firestore-triggered and on-demand refinement
- **Periodic run via Firestore trigger**: Add a Firestore function (or scheduler-backed HTTP) that writes a "run request" document containing batch scope (families, search windows). The runner listens to this collection and processes requests with leases to avoid duplicates.
- **Per-exercise fast path**: Support inserting a single `exercise_refinement_request` document with `exercise_id` (or alias), desired mode (`dry-run/apply`), and optional hints. The runner prioritizes these requests for short lead time, applies the same schema validation, and emits a targeted change plan.
- **Feedback loop**: On validation failure, the runner writes back a `validation_errors` array into the request doc, and the LLM is re-invoked with the errors + prior reasoning to self-correct while staying within the action schema.
- **Shared structure enforcement**: Both periodic and on-demand flows use the same action schema, policy middleware, and change journal. Positioning/description style rules live in the schema validator so that LLM-proposed descriptions are corrected or rejected with actionable feedback.
